...
void OpenMemory(CBinaryInputStream* bs, int size, void* pos)
{
    bs->stream_size = size;
    bs->field_0x4 = 2;
    bs->stream_pos = pos;
...
void Align(CBinaryInputStream* bs, int alignment)
{
    ...
    uint32_t size = (void*)((int)pos + alignment + -1 & -alignment);
    bs->stream_size = size;
    bs->stream_pos = bs->stream_pos - (int)((int)size - (int)stream_pos);
    return;
}
...
void ReadStringSw(CBinaryInputStream* bs, char *dst0)
{
