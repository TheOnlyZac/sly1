...
int AddMurrayEnemy(MURRAY *pmurray, ALO *paloEnemy) {
    ...
    if ((paloEnemy != 0) && (FIsLoInWorld__FP2LO(paloEnemy) != 0)) {
        if ((FIsBasicDerivedFrom__FP5BASIC3CID(paloEnemy, CID_STEPGUARD) == 0) || (STRUCT_OFFSET(paloEnemy, 0x724, SGS) != SGS_Dying)) {
...
int CheckMurrayStunOrDying(MURRAY *pmurray) {
    ...
    switch (sgs) {
        case SGS_Stun:   return 1;
        case SGS_Dying:     return 2;
        default:            return 0;
    }
}
